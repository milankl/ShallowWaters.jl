@with_kw struct Grid{T<:AbstractFloat}
    ...
    x_q::AbstractVector = if bc == "periodic" G.x_u else Δ*Array(1:nx+1) .- Δ end
    ...
    x_q_halo::AbstractVector = if bc == "periodic" G.x_u_halo else Δ*Array(-1:nx+3) .- Δ end
...
function Grid{T}(P::Parameter)
    @unpack T,nx,Lx,L_ratio = P
    @unpack bc,g,H,cfl = P
    @unpack Ndays,nstep_diff,nstep_advcor = P
    @unpack Uadv,output_dt = P
    @unpack ϕ,ω,R = P

    return Grid(T=T,nx=nx,Lx=Lx,L_ratio=L_ratio,bc=bc,g=g,H=H,cfl=cfl,Ndays=ndays,
