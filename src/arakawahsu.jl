function AHα(α::AbstractMatrix,q::AbstractMatrix)
    ...
    @inbounds for j ∈ 1:n
        for i ∈ 1:m
            α[i,j] = one_third*(q[i,j] + q[i,j+1] + q[i+1,j+1])
        end
    end
end
...
function AHβ(β::AbstractMatrix,q::AbstractMatrix)
    ...
    m,n = size(β)
    @boundscheck (m-1,n) == size(q) || throw(BoundsError())

    @inbounds for j ∈ 1:n
        β[1,j] = one_third*(q[1,j] + q[1,j+1] + q[end,j+1])
        for i ∈ 2:m
            β[i,j] = one_third*(q[i,j] + q[i,j+1] + q[i-1,j+1])
        end
    end
end
...
function AHγ(γ::AbstractMatrix,q::AbstractMatrix)
    ...
    @inbounds for j ∈ 1:n
        γ[1,j] = one_third*(q[1,j] + q[1,j+1] + q[end,j])
        for i ∈ 2:m
            γ[i,j] = one_third*(q[i,j] + q[i-1,j] + q[i,j+1])
        end
    end
end
...
function AHδ(δ::AbstractMatrix,q::AbstractMatrix)
    ...
    @inbounds for j ∈ 1:n
        for i ∈ 1:m
            δ[i,j] = one_third*(q[i,j] + q[i,j+1] + q[i+1,j])
        end
    end
end
